...
- the focused Mindoro-focused provenance box is kept separate at `118.751-124.305E / 10.620-16.026N` as Study Box `1`, while the publication package now includes a shared study-box reference figure that foregrounds Study Box `2` (`mindoro_case_domain`) plus Study Box `4` (the prototype_2016 historical-origin box) and preserves Study Boxes `1` and `3` as archived secondary references instead of collapsing everything into one extent
